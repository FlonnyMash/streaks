...
import type { OnboardingRoutineOption } from '@/lib/onboardingRoutines'

export interface CustomRoutineOption {
  id: string
  label: string
  emoji: string
}

interface RoutinePillGridProps {
  options: OnboardingRoutineOption[]
  selected: string[]
...
/** Multi-select emoji pills for picking routine tasks during onboarding, plus a custom "add your own" pill. */
export function RoutinePillGrid({
  options,
  selected,
  onToggle,
  customOptions,
  ...
  function submitCustom() {
    ...
    if (!label) return
    onAddCustom(label, customEmoji || DEFAULT_CUSTOM_EMOJI)
    closeCustomForm()
  }
    ...
    <div className="flex flex-col gap-3 w-full">
      <div className="flex flex-wrap gap-2.5 justify-center">
        ...
        {customOptions.map((option) => (
          <span
            key={option.id}
            className="flex items-center gap-2 h-11 pl-3 pr-2 rounded-full text-[14px] font-medium bg-accent-blue/15 ring-2 ring-accent-blue text-accent-blue"
          >
            <span className="text-lg leading-none">{option.emoji}</span>
            {option.label}
            <button

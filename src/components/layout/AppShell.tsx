...
import { WeekendRoutinePrompt } from '@/components/calendar/WeekendRoutinePrompt'
    ...
    <OfflineSyncProvider>
      <TodoTimerProvider>
        <div className="min-h-dvh flex flex-col app-shell">
          <PasskeySetupPrompt />
          <PushSetupPrompt />
          <WeekendRoutinePrompt />
          <GlassNavbar />
          <MobileTopBar />
          <main className="flex-1 mx-auto w-full max-w-5xl safe-x [--safe-x-pad:1rem] app-desktop:[--safe-x-pad:1.5rem] pt-[calc(3.5rem+env(safe-area-inset-top,0px))] max-sm:landscape:pt-[calc(4.75rem+env(safe-area-inset-top,0px))] sm:pt-[calc(6.75rem+env(safe-area-inset-top,0px))] pb-[calc(5.5rem+var(--safe-area-bottom,env(safe-area-inset-bottom,0px)))] app-desktop:pb-8">
            <Outlet />
